...

{
    ...
    public class PacienteController : ControllerBase
    {
        ...
        // POST: api/Paciente
        [HttpPost]
        public async Task<IActionResult> AdicionarPaciente([FromBody] JsonElement json)
        {
            try
            {
                // Extrair valores do JSON
                string cpfPaci = json.GetProperty("CpfPaci").GetString() ?? throw new ArgumentNullException("CpfPaci não pode ser nulo.");
                string nomePaci = json.GetProperty("NomePaci").GetString() ?? throw new ArgumentNullException("NomePaci não pode ser nulo.");
                string sobrenomePaci = json.GetProperty("SobrenomePaci").GetString() ?? throw new ArgumentNullException("SobrenomePaci não pode ser nulo.");
                DateOnly nascimentoPaci = DateOnly.FromDateTime(json.GetProperty("Nascimentopaci").GetDateTime());
                string generoPaci = json.GetProperty("GeneroPaci").GetString() ?? throw new ArgumentNullException("GeneroPaci não pode ser nulo.");
                string emailPaci = json.GetProperty("EmailPaci").GetString() ?? throw new ArgumentNullException("EmailPaci não pode ser nulo.");
                string telefonePaci = json.GetProperty("TelefonePaci").GetString() ?? throw new ArgumentNullException("TelefonePaci não pode ser nulo.");
                string senhaPaci = json.GetProperty("Senhapaci").GetString() ?? throw new ArgumentNullException("Senhapaci não pode ser nulo.");
                ...
                var paciente = new Paciente
                {
                    CpfPaci = cpfPaci,
                    NomePaci = nomePaci,
                    SobrenomePaci = sobrenomePaci,
                    NascimentoPaci = nascimentoPaci,
                    GeneroPaci = generoPaci,
                    EmailPaci = emailPaci,
                    TelefonePaci = telefonePaci,
                    SenhaPaci = senhaPaci
                };

                // Inserindo os dados no banco de dados
                var result = await InserirPaciente(paciente);
                ...
                {
                    return BadRequest("Erro ao adicionar o paciente.");
                }
            }
